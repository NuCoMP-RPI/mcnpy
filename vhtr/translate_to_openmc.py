...
def mcnp_to_openmc(deck:mp.Deck):
            ...
            if cell.fill is None:
                if cell.material is None:
                    ...
                    # Material has at least one density defined for it.
                        ...
                        print(density, openmc_materials)
                        exist = False
                        # Check if alternate density has already been used.
                        for m in openmc_materials:
                            if openmc_materials[m].name == str(cell.material.name):
    ...
    '''
    q = 0
    #print('LAT_UNIVERSES', lat_universes)
    for k in lat_universes:
        ...
        #print(type(lat_fill), openmc_universes[k])
        # Lattice dimensions
        dims = lat_dims[k]
        ...
        for j in openmc_cells:
            if openmc_cells[j].fill == openmc_universes[k]:
                ...
                openmc_cells[j].fill = lat_fill
            """else:
                print(openmc_cells[j].fill, type(openmc_cells[j].fill))"""
